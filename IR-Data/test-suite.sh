...
prog_10=../Out/x64/Release/irmpAnalyze10kHz
prog_15=../Out/x64/Release/irmpAnalyze15kHz
prog_20=../Out/x64/Release/irmpAnalyze20kHz
...
do
    ...
    if $prog_10 -v < $j | grep -q error
    then
	$prog_10 -v < $j | grep error
	echo "test failed"
	exit 1
    else
	if $prog_10 -v < $j | grep -q checked
	then
	    echo "checked!"
	else
	    echo "successful"
...
do
    echo -n "testing $j (16 BIT) ... "
    if $prog_15 -v < $j | grep -q error
    then
	$prog_15 -v < $j | grep error
	echo "test failed"
	exit 1
    else
	if $prog_15 -v < $j | grep -q checked
	then
	    echo "checked!"
	else
	    echo "successful"
...
do
    echo -n "testing $j (16 BIT) ... "
    if $prog_20 -v < $j | grep -q error
    then
	$prog_20 -v < $j | grep error
	echo "test failed"
	exit 1
    else
	if $prog_20 -v < $j | grep -q checked
	then
	    echo "checked!"
	else
	    echo "successful"
